...
export async function build({ clean, logLevel, outDir, publicDir, searchIndex }) {
    ...
    await build({
        ...
        hooks: {
            ...
            onBundleEnd({ error }) {
                ...
                if (useLogger)
                    spinner.client.succeed('bundles built');
                else
                    logger.info('');
            },
            onPrerenderStart() {
                if (useLogger)
                    spinner.prerender.start();
            },
            onPrerenderEnd({ error }) {
                if (error) {
                    if (useLogger)
                        spinner.client.fail(`prerendering failed: ${error.message}`);
                    process.exit(1);
                }
                if (useLogger)
                    spinner.prerender.succeed('prerendered pages');
            },
            onScriptsEnd() {
                if (!useLogger)
                    logger.info('');
